...
float PedirInformacion(int n, costoPorArticulo articulo[]){
    string nombre;//Se declara la variable que recibirá el nombre del artículo
    for (int i = 0; i < n; i++)
    {
        cout<<"Nombre del articulo a pagar: ";
        getline(cin,nombre, '\n');//Se emplea el getline por si se requiere un nombre con espacios, como por ejemplo: Leche de soya
        strncpy(articulo[i].nombreArticulo,nombre.c_str( ),longCad);/*Se copia el nombre ingresado en el campo del arreglo: articulo[i]
        indicando que no debe de exceder los 20 espacios establecidos en la constante longCad*/
        articulo[i].nombreArticulo[longCad]='\0';//el último espacio(21) se guarda para el carácter especial de fin de cadena \0
...
float CostoxArticulo(int n, costoPorArticulo articulo[]){
    for (int i = 0; i < n; i++)
    {
        ...
        //El articulo[i].costoPorArt irá almacenando el producto resultante para obtener el costo total por artículo
    }
}
...
>>>>>>> 93266c535a19b4b60a4eac3642ffe55c70be5bd6
